 ...
 */
public class SpinField extends JPanel implements ChangeListener, CaretListener, ActionListener,
		FocusListener {
	 ...
	 * value 0.
	 */
	public SpinField(int min, int max) {
		super();
		setName("JSpinField");
		this.min = min;
		if (max < min)
		...
		spinner = new Spinner() {
			...
			private JTextField textField = new TextField();
	...
	protected void setValue(int newValue, boolean updateTextField, boolean firePropertyChange) {
		...
		if (updateTextField) {
			textField.setText(Integer.toString(value));
			textField.setForeground(new Color(201, 38, 84));
		}
